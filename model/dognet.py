...
class DoGNet_Without_Attention(nn.Module):
    def __init__(self, num_classes=10, in_channels=3, num_pyramid_levels=3, drop_out=0.5):  # Updated num_classes
        super(DoGNet_Without_Attention, self).__init__()
        self.num_pyramid_levels = num_pyramid_levels
...
class DoGNet_Without_Dog(nn.Module):
    def __init__(self, num_classes=10, in_channels=3, num_pyramid_levels=3, drop_out=0.5):  # Updated num_classes
        super(DoGNet_Without_Dog, self).__init__()
        self.num_pyramid_levels = num_pyramid_levels

        self.channels = [16, 32, 32, 64, 64, 128, 128, 256, 256]

        self.conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=self.channels[0], kernel_size=3, stride=1, padding=1)
        self.conv2 = nn.Conv2d(
            in_channels=self.channels[0], out_channels=self.channels[1], kernel_size=5, stride=1, padding=2)
        self.conv3 = nn.Conv2d(
            in_channels=self.channels[1], out_channels=self.channels[2], kernel_size=3, stride=1, padding=1)
        ...
        self.bn1 = nn.BatchNorm2d(self.channels[2])
        self.align_conv1 = nn.Conv2d(
            in_channels=in_channels, out_channels=self.channels[2], kernel_size=3, stride=1, padding=1)
        self.atten1 = FeatureMapAttention(2, self.channels[2])
        ...
        self.bn2 = nn.BatchNorm2d(self.channels[5])
        self.align_conv2 = nn.Conv2d(
            in_channels=in_channels, out_channels=self.channels[5], kernel_size=3, stride=1, padding=1)
        self.atten2 = FeatureMapAttention(2, self.channels[5])
    ...
    def forward(self, x):
        ...
        x = self.max_pooling2d(x)
        res = self.relu(self.align_conv1(pyramid[1]))
        x = self.atten1([x, res])
        ...
        x = self.max_pooling2d(x)
        res = self.relu(self.align_conv2(pyramid[2]))
        x = self.atten2([x, res])
...
class DoGNet_Single_Scale(nn.Module):
    def __init__(self, num_classes=10, in_channels=3, num_pyramid_levels=3, drop_out=0.5):  # Updated num_classes
        super(DoGNet_Single_Scale, self).__init__()
        self.num_pyramid_levels = num_pyramid_levels

        self.channels = [16, 32, 32, 64, 64, 128, 128, 256, 256]

        # First custom convolutional layer  
        self.dog = DoG(in_channels=in_channels, out_channels=self.channels[0],
                       kernel_size=5, stride=1, padding=2)
...
if __name__ == "__main__":
    ...
    model = DoGNet_Single_Scale(in_channels=in_channels).to('cpu')

    # Forward pass
    output = model(x)
    print(f"Output shape: {output.shape}")  # Expected: (2, 16, 64, 64)

    model = DoGNet_Without_Attention(in_channels=in_channels).to('cpu')

    # Forward pass
    output = model(x)
    print(f"Output shape: {output.shape}")  # Expected: (2, 16, 64, 64)

    model = DoGNet_Without_Dog(in_channels=in_channels).to('cpu')
